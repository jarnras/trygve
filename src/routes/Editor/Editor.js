...
class Editor extends React.Component {
  ...
  async publishEvent(isDraft = false) {
    ...
    if (this.props.params.id === 'new') {
      try {
        ...
        browserHistory.push(`${process.env.PREFIX_URL}/admin/edit/${res.id}`);
      } catch (error) {
        console.log(error);
  ...
  render() {
      ...
      return (
        <div className="event-editor">
          <div className="event-editor--loading-container">
            ...
            <Link to={`${process.env.PREFIX_URL}/admin/`}>Palaa tapahtumalistaukseen</Link>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="event-editor">
        <Link to={`${process.env.PREFIX_URL}/admin`}>&#8592; Takaisin</Link>
        <Formsy.Form
