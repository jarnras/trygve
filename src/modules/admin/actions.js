...
export const getEventsAsync = () => (dispatch, getState) => {
  ...
  request('GET', `${process.env.PREFIX_URL}/api/admin/events`, {
...
export const getUsersAsync = () => (dispatch, getState) => {
  ...
  request('GET', `${process.env.PREFIX_URL}/api/users`, {
    headers: { Authorization: accessToken },
  })
    .then(res => JSON.parse(res.body))
    .then((res) => {
...
export const createUserAsync = data => (dispatch, getState) => {
  const accessToken = getState().admin.accessToken;
  return request('POST', `${process.env.PREFIX_URL}/api/users`, {
    headers: { Authorization: accessToken },
...
export const login = (email, password) => (dispatch) => {
  ...
  request('POST', `${process.env.PREFIX_URL}/api/authentication`, {
    ...
    .then((res) => {
      ...
      dispatch(push(`${process.env.PREFIX_URL}/admin`));
    })
    .catch((error) => {
...
export const redirectToLogin = () => (dispatch) => {
  dispatch(clearState());
  dispatch(push(`${process.env.PREFIX_URL}/login`));
};

export const deleteEventAsync = id => (dispatch, getState) => {
  const accessToken = getState().admin.accessToken;

  return request('DELETE', `${process.env.PREFIX_URL}/api/admin/events/${id}`, {
    headers: { Authorization: accessToken },
  })
    .then(() => true)
    .catch((error) => {
...
export const deleteUserAsync = id => (dispatch, getState) => {
  ...
  return request('DELETE', `${process.env.PREFIX_URL}/api/users/${id}`, {
    headers: { Authorization: accessToken },
  })
    .then(() => true)
    .catch((error) => {
...
export const deleteSignupAsync = (id, eventId) => (dispatch, getState) => {
  const accessToken = getState().admin.accessToken;
  return request('DELETE', `${process.env.PREFIX_URL}/api/admin/signups/${id}`, {
    headers: { Authorization: accessToken },
  })
    .then(res => JSON.parse(res.body))
    .then(() => {
