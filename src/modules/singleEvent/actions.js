...
export const updateEventAsync = eventId => (dispatch) => {
  ...
  return request('GET', `${process.env.PREFIX_URL}/api/events/${eventId}`)
...
export const attachPositionAsync = quotaId => (dispatch) => {
  ...
  return request('POST', `${process.env.PREFIX_URL}/api/signups`, { json: { quotaId } })
    .then(res => JSON.parse(res.body))
    .then((res) => {
...
export const completeSignupAsync = (signupId, data) => (dispatch) => {
  ...
  return request('PATCH', `${process.env.PREFIX_URL}/api/signups/${signupId}`, {
    json: {
...
export const cancelSignupAsync = (signupId, editToken) => (dispatch) => {
  dispatch(setSignupLoading());
  return request('DELETE', `${process.env.PREFIX_URL}/api/signups/${signupId}?editToken=${editToken}`)
    .then(res => JSON.parse(res.body))
    .then(() => {
      dispatch(setSignup({}));
    })
